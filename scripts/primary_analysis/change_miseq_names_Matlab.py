### From variant_pipeline; modified by ALV on 6/11/19 to accommodate filenames from core
...
for zipfilename in glob.glob(s + "*.fastq.gz"):
    ...
    good_name = name[0].split("_")[0]
    print(name[1])
    lane_junk = name[1].split("_")
    read_number=lane_junk[0]
    fastq_number=lane_junk[1][2]
